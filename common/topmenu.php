<div class="container-flex topSocial">
    <ul class="container d-flex flex-row justify-content-between m-auto p-0 align-items-center">
        <li> <i class="fas fa-mobile-alt me-2" aria-hidden="true"></i> +91 9424777821 / 9755544329 </li>
        <li> <i class="fas fa-envelope  me-2"></i> info@nandankids.in </li>
        <li> <i class="fas fa-clock  me-2" aria-hidden="true"></i> Mon — Fri: 8.00 AM — 18.00 PM </li>
        
        <li class="nandan-socl-wrapr d-inline-flex flex-row ">
            <a class="nandan-socl-fb" href="" rel="nofollow" target="_blank"><i class="fab fa-facebook-f"></i></a>
            <a class="nandan-socl-twit" href="" rel="nofollow" target="_blank"><i class="fab fa-twitter"></i></a>
            <a class="nandan-socl-lnkd" href="" rel="nofollow" target="_blank"><i class="fab fa-linkedin"></i></a>
            <a class="nandan-socl-insta" href="" rel="nofollow" target="_blank"><i class="fab fa-instagram"></i></a>
            <a class="nandan-socl-utub" href="" rel="nofollow" target="_blank"><i class="fab fa-youtube"></i></a>
        </li> 
        
    </ul>
</div>  

<nav class="topMenu nav container m-auto ps-0 pe-0"> 
    <a class="logoText shadow-sm p-3 mb-5 bg-body rounded" href="index.php"><img src="./images/logo.png" alt="Nandan School Logo"></a> 
    <a href="#" id="topbar-menu-icon" class="menu-icon"><i class="fas fa-bars"></i> </a>
    <ul id="topbar-menu" class="topbarMenu d-flex flex-row justify-content-end align-items-center">
        <li> <a href="index.php"> Home </a> </li>
        <li> <a href="#"> About Us </a> </li>
        <li> <a href="#"> Facilities </a> </li> 
        <li> <a href="#"> Staff </a> </li>             
        <li> <a href="#"> Admission </a> </li>
        <li> <a href="#"> Gallery </a> </li>
        <li> <a href="#"> Alumni </a> </li>
        <li> <a href="#"> Staff </a> </li>
        <li class="contactUs"> <a href="#"> Inquiry </a> </li>
    </ul>   
</nav>

 